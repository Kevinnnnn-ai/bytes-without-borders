...
- 2026-07-13: When touching nav/footer, remember chrome is duplicated across all 9 pages in `docs/` — the pytest suite catches broken links and missing i18n keys but not visual drift; eyeball one root page and one lessons/ page.
- 2026-07-13: CSS gotcha bitten twice: any rule that sets `display` on an element defeats the `hidden` attribute. Guards exist for `.filter-bar[hidden]` and `.form-grid[hidden]`; add one for any new display-styled element that ships `hidden`.
